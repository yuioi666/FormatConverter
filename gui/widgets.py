...
class LogPanel(ttk.Labelframe):
    ...
    def __init__(self, parent, **kwargs):
        ...
        self._btn_toggle = ttk.Button(
            toolbar, text="展开日志", bootstyle="secondary-outline", width=10,
        ...
        scroll = ttk.Scrollbar(
            self._text_frame, orient=VERTICAL, command=self._text.yview,
        )
        scroll.pack(side=RIGHT, fill=Y)
        self._text.configure(yscrollcommand=scroll.set)

        # 注册日志回调
        logger.set_callback(self._enqueue_log_entry)
        self._log_after_id = self.after(50, self._drain_log_entries)
    ...
    def _toggle(self):
        ...
        if self._visible:
            self._text_frame.pack_forget()
            self._btn_toggle.config(text="展开日志")
            self._visible = False
        else:
            self._text_frame.pack(fill=BOTH, expand=True)
            self._btn_toggle.config(text="收起日志")
            self._visible = True
    ...
    def _enqueue_log_entry(self, entry: LogEntry):
        """允许任意线程安全投递日志。"""
        self._log_queue.put(entry)

    def _drain_log_entries(self):
        """由 Tk 主线程刷新日志控件。"""
        try:
            while True:
                self._append_log_entry(self._log_queue.get_nowait())
        except queue.Empty:
            pass
    ...
    def destroy(self):
        ...
        if getattr(self, "_log_after_id", None):
            try:
...
class DragDropBanner(ttk.Frame):
    ...
    def __init__(self, parent, **kwargs):
        kwargs.setdefault("bootstyle", "info")
        super().__init__(parent, **kwargs)

        self.label = ttk.Label(
            self,
            text="拖拽文件或文件夹到这里",
            font=scaled_font("微软雅黑", 11, "bold"),
        ...
        self.hint = ttk.Label(
            self,
            text="将按当前转换类型自动筛选可用文件",
            font=scaled_font("微软雅黑", 8),
            bootstyle=(INVERSE, INFO),
        )
        self.hint.pack(pady=(1, 8))

    def set_formats(self, source: str, target: str):
        """更新拖放区的当前转换提示。"""
        self.hint.config(text=f"当前：{source}  →  {target}，将自动筛选可用文件")
